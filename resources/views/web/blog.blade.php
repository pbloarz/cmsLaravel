				...
				<div class="col-xs-12 col-sm-9 col-md-9 rht-col">
                    @foreach ($posts as $post)
                        <div class="blog-post {{$loop->first ? '' : 'outer-top-bd' }} wow fadeInUp">
                            <a href="blog-details.html"><img class="img-responsive" src="{{$post->image->url}}" alt=""></a>
                            <h1><a href="{{route('web.blogDetails',$post->id)}}" >{{$post->name}}</a></h1>
                            <span class="author">{{$post->user->name}}</span>
                            <span class="review">{{$post->comments_count}} Comentarios</span>
                            <span class="date-time"> {{$post->updated_at->format('d/m/y')}}</span>
                            <p> {!!htmlspecialchars_decode($post->body)!!}</p>
                            <a href="{{route('web.blogDetails',$post->id)}}" class="btn btn-upper btn-primary read-more">read more</a>
                        </div>
                    @endforeach
                    ...
                    <div class="sidebar-module-container">
						<div class="search-area outer-bottom-small">
                            ...
                            <div class="home-banner outer-top-n outer-bottom-xs">
                                <img src="marazzo/assets/images/banners/LHS-banner.jpg" alt="Image">
                            </div>
                                <!-- ==============================================CATEGORY============================================== -->
